...
export default function HandwrittenMessage() {
  ...
  const sendEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (messageType === 'handwritten') {
        ...
        formData.append('message', 'A handwritten message from the wedding website');
  ...
  return (
    <div className="select-none">
      ...
      <div className="bg-white/50 backdrop-blur-sm border border-white/30 rounded-2xl p-6 md:p-8 shadow-lg">
        <div className="flex flex-col md:flex-row gap-4 justify-center mb-6">
          <button
            type="button"
            onClick={() => setMessageType('handwritten')}
            className={`px-6 py-3 rounded-lg font-medium transition-all ${messageType === 'handwritten'
                ? 'bg-gradient-to-r from-pink-300 to-orange-300 text-white shadow-lg'
                : 'bg-white/50 text-gray-700 hover:bg-white/70'
              }`}
          >
            {t('handwrittenMessage')}
          </button>
          <button
            type="button"
            onClick={() => setMessageType('normal')}
            className={`px-6 py-3 rounded-lg font-medium transition-all ${messageType === 'normal'
                ? 'bg-gradient-to-r from-pink-300 to-orange-300 text-white shadow-lg'
                : 'bg-white/50 text-gray-700 hover:bg-white/70'
              }`}
          >
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {penColors.map((pen) => (
                      <button
                        ...
                        className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 flex-shrink-0 ${currentColor === pen.color ? 'border-gray-800' : 'border-gray-300'
                          }`}
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {penWidths.map((pen) => (
                      <button
                        ...
                        className={`px-2 py-1 text-[10px] sm:text-xs rounded-md transition-colors whitespace-nowrap ${currentWidth === pen.width
